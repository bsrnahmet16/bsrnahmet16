...
DURATIONS={16:5.220,17:5.600,18:5.380}
...
def import_rig(path,name,height,loc):
 ...
 for arm in [o for o in objs if o.type=='ARMATURE']:
  ...
  if act:
   ...
   lo,hi=act.frame_range;span=max(1.0,hi-lo);factor=max(1.0,FRAMES-1)/span
...
pofi_loc={16:(-2.55,-.05,-.46),17:(-2.55,-.05,-.46),18:(-2.55,-.05,-.46)}[SCENE]
pofi=import_rig('assets/Zipzi_3D_rigged.glb','Zipzi',5.25,pofi_loc);pofi.rotation_euler[2]=math.radians(-5)
...
for o in (back,front):
 if False:
  ...
  if SCENE in (16,18):
   # High-contrast teaching and clapping beats.
   for f in (18,58,100):
