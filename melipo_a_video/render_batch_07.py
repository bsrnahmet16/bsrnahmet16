...
DURATIONS={19:6.180,20:5.000,21:5.640}
  ...
  if act:
   ...
   if mouth:
    for f in range(10,FRAMES,14):
     ...
     mouth.scale=(1.0,.95,1.07);mouth.keyframe_insert('scale',frame=f)
     mouth.scale=(1,1,1);mouth.keyframe_insert('scale',frame=min(FRAMES,f+4))
 return root

pofi_loc={19:(-3.05,-.05,-.46),20:(-3.05,-.05,-.46),21:(-3.05,-.05,-.46)}[SCENE]
pofi=import_rig('assets/Luma_3D_rigged.glb','Luma',4.75,pofi_loc);pofi.rotation_euler[2]=math.radians(-5)
...
if SCENE not in (19,20):
 ...
 base=(1,1,1);tree.scale=(0,0,0) if SCENE==19 else base;tree.keyframe_insert('scale',frame=1)
 if SCENE==19:
  tree.keyframe_insert('scale',frame=max(1,tree_reveal-4));tree.scale=base;tree.keyframe_insert('scale',frame=tree_reveal)
 tree.rotation_euler[1]=math.radians(-1.5);tree.keyframe_insert('rotation_euler',frame=max(1,tree_reveal))
 tree.rotation_euler[1]=math.radians(1.5);tree.keyframe_insert('rotation_euler',frame=max(tree_reveal+6,FRAMES//2))
...
if SCENE!=21:
 ...
 moon.scale=(1,1,1);moon.keyframe_insert('scale',frame=moon_reveal)
 moon.rotation_euler[1]=math.radians(-4);moon.keyframe_insert('rotation_euler',frame=moon_reveal)
 moon.rotation_euler[1]=math.radians(4);moon.keyframe_insert('rotation_euler',frame=FRAMES)
...
for obj in bpy.data.objects:
 if obj.type=='MESH' and not is_under(obj,pofi) and not is_under(obj,bee) and not is_under(obj,tree) and not is_under(obj,moon): obj.hide_render=True
...
if SCENE==21:
 tint=cn.new('CompositorNodeHueSat');tint.inputs['Saturation'].default_value=.72;tint.inputs['Value'].default_value=.40
 blue=cn.new('CompositorNodeMixRGB');blue.blend_type='MULTIPLY';blue.inputs[0].default_value=.52;blue.inputs[2].default_value=(.22,.38,.70,1)
 cl.new(scale.outputs['Image'],tint.inputs['Image']);cl.new(tint.outputs['Image'],blue.inputs[1]);cl.new(blue.outputs['Image'],over.inputs[1])
else:
