...
DURATIONS={22:5.360,23:5.550,24:5.930}
...
pofi_loc={22:(-2.75,-.05,-.46),23:(-3.05,-.05,-.46),24:(-3.25,-.05,-.46)}[SCENE]
pofi_height=5.20 if SCENE==22 else 4.55
...
car_reveal=round((125.53-120.28)*30) if SCENE==23 else 1
if SCENE==22:
 ...
 car.scale=(.80,.80,.80);car.keyframe_insert('scale',frame=car_reveal)
 car.keyframe_insert('location',frame=car_reveal);car.location.x=.20;car.keyframe_insert('location',frame=FRAMES)
else:
 car.scale=(.74,.74,.74);car.keyframe_insert('scale',frame=1)
 car.keyframe_insert('location',frame=1);car.location.x=.20;car.keyframe_insert('location',frame=FRAMES)
...
for o in (back,front):
 if False:
  ...
  if SCENE==22:
   # A readable teaching pulse on the sung "Alkislarla A" beat.
   for f in (18,58,100):
...
reveal=round((127.07-125.83)*30) if SCENE==24 else 1
if SCENE in (22,23):
...
for obj in bpy.data.objects:
 if obj.type=='MESH' and not is_under(obj,pofi) and not is_under(obj,bee) and not is_under(obj,lion) and not is_under(obj,car): obj.hide_render=True
