...
DURATIONS={10:5.350,11:5.330,12:5.490}
...
pofi_loc={10:(-2.75,-.05,-.46),11:(-3.05,-.05,-.46),12:(-3.05,-.05,-.46)}[SCENE]
pofi_height=5.20 if SCENE==10 else 4.70
...
lion.location=(2.35,.10,-.44)
reveal=round((58.69-54.88)*30) if SCENE==11 else 1
if SCENE==10:
 lion.scale=(0,0,0);lion.keyframe_insert('scale',frame=1);lion.keyframe_insert('scale',frame=FRAMES)
else:
 lion.scale=(0,0,0) if SCENE==11 else (1,1,1)
 lion.keyframe_insert('scale',frame=1)
 if SCENE==11:
  lion.keyframe_insert('scale',frame=max(1,reveal-4));lion.scale=(1,1,1);lion.keyframe_insert('scale',frame=reveal)
 lion.keyframe_insert('location',frame=max(1,reveal))
...
for obj in bpy.data.objects:
 if obj.type=='MESH' and not is_under(obj,pofi) and not is_under(obj,bee) and not is_under(obj,lion): obj.hide_render=True
