...
kind = "group"
SCENE=int(sys.argv[sys.argv.index("--")+1]) if "--" in sys.argv else 28
DURATIONS={28:4.960,29:5.336,30:5.464}
...
# Preserve only canonical GLB characters and exact typography over the approved premium plate.
...
for obj in bpy.data.objects:
 if obj.type=="MESH" and not any(under_root(obj,r) for r in char_roots):
  obj.hide_render=True
scene.render.film_transparent=True
scene.use_nodes=True
cn=scene.node_tree.nodes;cl=scene.node_tree.links;cn.clear()
rl=cn.new("CompositorNodeRLayers");bg=cn.new("CompositorNodeImage")
bg.image=bpy.data.images.load("assets/premium_meadow_background_v1.png")
...
cl.new(bg.outputs["Image"],scale.inputs["Image"]);cl.new(scale.outputs["Image"],over.inputs[1])
cl.new(rl.outputs["Image"],over.inputs[2]);cl.new(over.outputs["Image"],comp.inputs["Image"])
